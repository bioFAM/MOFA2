...
#' @rdname plot_training_curve_factors
#' @name plot_training_curve_factors
...
#' All training statistics, including the number of active factors, can be fetch from the training_stats slot of \code{\link{BioFAModel}} .
#' @param object a \code{\link{BioFAModel}} object.
#' @import ggplot2 scales
#' @export

plot_training_curve_factors <- function(object) {
  
  # Sanity checks
  if (class(object) != "BioFAModel") { stop("'object' has to be an instance of BioFAModel") }
  
  # Collect training statistics
  idx  <- seq(1 + object@training_options$start_drop,
              length(object@training_stats$activeK),
              object@training_options$freq_drop)
  stat <- object@training_stats$activeK[idx] 
...
#' @name plot_training_curve_ELBO
#' @rdname plot_training_curve_ELBO
#' @param object a \code{\link{BioFAModel}} object.
...
#' All Training statistics, including the ELBO, can be fetch from the training_stats slot of \code{\link{BioFAModel}} .
#' @import ggplot2 scales
#' @export

plot_training_curve_ELBO <- function(object, log = F) {
  
  # Sanity checks
  if (class(object) != "BioFAModel") { stop("'object' has to be an instance of BioFAModel") }
  
  # Fetch ELBO from training_stats  
  idx = seq(1,length(object@training_stats$elbo),object@training_options$elbofreq)
  stat = object@training_stats$elbo[idx]
