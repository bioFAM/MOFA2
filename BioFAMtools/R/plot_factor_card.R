...
plot_factor_card <- function(object, factor, color_by="group", gene_sets=NULL) {
  ...
  #plot 1 : expression across samples
  ...
  df_var_res = data.frame(groups,views,var_explained)
  ...
  if (N_views==1){
    list_plots = list(plotZ,plotViews[[1]][[1]],plotVar,plotViews[[1]][[2]])
    do.call("grid.arrange", c(list_plots,ncol=2,nrow=2))
  }
  else{
    if  (N_views==2){
      list_plots = list(plotZ,plotViews[[1]][[1]],plotViews[[1]][[2]])
      list_plots = append(list_plots,plotVar,plotViews[[2]][[1]],plotViews[[2]][[2]])
