...
class initModel(object):
    ...
    def initSZ(self, pmean_S0=0., pmean_S1=0., pvar_S0=1., pvar_S1=1., ptheta=1., qmean_S0=0., qmean_S1=0., qvar_S0=1.,
              qvar_S1=1., qtheta=1., qEZ_S0=0., qEZ_S1=0., qES=1.):
        ...
        if isinstance(qmean_S1, str):
            ...
            if qmean_S1 == "random":  # random
                qmean_S1_tmp = stats.norm.rvs(loc=0, scale=1, size=(self.N, self.K))
            else:
                print("%s initialisation not implemented for Z" % qmean_S1)
                exit()

        elif isinstance(qmean_S1, s.ndarray):
            assert qmean_S1.shape == (self.N, self.K), "Wrong dimensionality"

        elif isinstance(qmean_S1, (int, float)):
            qmean_S1_tmp = s.ones((self.N, self.K)) * qmean_S1
    ...
    def initW(self, pmean=0., pvar=1., qmean="random", qvar=1., qE=None, qE2=None, covariates=None,
        ...
        for m in range(self.M):
            ...
            # Add covariates
            if covariates is not None:
                covariates_m = covariates.copy()

                assert scale_covariates != None, "If you use covariates also define data_opts['scale_covariates']"

                # Select indices for covariates
                idx_covariates = s.array(range(covariates_m.shape[1]))

                # Center and scale the covariates to match the prior distribution N(0,1)
                scale_covariates = s.array(scale_covariates)
                covariates_m[:, scale_covariates] = (covariates_m[:, scale_covariates] - s.nanmean(
                    covariates_m[:, scale_covariates], axis=0)) / s.nanstd(covariates_m[:, scale_covariates], axis=0)

                # Set to zero the missing values in the covariates
                covariates_m[s.isnan(covariates_m)] = 0.
                qmean[:, idx_covariates] = covariates_m
            ...
            # Initialise the node
            W_list[m] = W_Node(dim=(self.D[m], self.K), pmean=pmean_m, pvar=pvar_m, qmean=qmean_m, qvar=qvar_m, qE=qE, qE2=qE2,
    ...
    def initThetaLearnW_mk(self, pa=1., pb=1., qa=1., qb=1., qE=1.):
