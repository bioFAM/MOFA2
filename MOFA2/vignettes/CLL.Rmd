...
Information on other columns can be obtained using`?patmeta`.
```{r}
library(BloodCancerMultiOmics2017)
data("patmeta")
# extract metadata for patients with omics data
CLL_metadata <- patmeta[colnames(CLL_data$Drugs),] 
CLL_metadata <- rownames_to_column(CLL_metadata, "sample")
CLL_metadata <- rename(CLL_metadata, age = Age4Main)
head(CLL_metadata)
```
