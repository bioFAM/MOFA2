...
- **Age**: age in years
- **T5**: time (in years) which passed from taking the sample to the next treatment
- **T6**: time (in years) which passed from taking the sample to patients' death
...
MOFAobject <- load_model("~/Downloads/MOFA2_CLL.hdf5")
```
...
# Building predictive models of clinical outcome (TO-DO)
The factors inferred by MOFA can be related to clinical outcomes such as time to treatment or survival times. As this type of data is censored (not for all samples we have already observed the event) we will use Cox models for this purpose. In a Cox proportional hazards model we model the hazard of an event ocurring (e.g. death or treatment) as a function in covariates (here the factors). If a factor has a influence on the surivival time or time to treatment it will recevie a high absoulte coefficient in the Cox model:

- If the coefficient is positive, samples with large factor values have an increased hazard (of death or treatment) compared to  with a low factor values
- If the coefficient is negative, samples with low factor values have an increased hazard compared to  with a large factor values
...
Let's take time to treatment as an example here. The `samples_metadata` contains the follow-up times per sample in years in column `T5` and the column `treatedAfter` indicated whether a treatment occured. Using the MOFA factors as covariates (in a matrix format) we can driectly relate them to the Surv object using `coxph`.

```{r}
library(survival)
SurvObject <-Surv(MOFAobject@samples_metadata$T5, MOFAobject@samples_metadata$treatedAfter)
Z <- get_factors(MOFAobject)[[1]]
fit <- coxph(SurvObject  ~  Z) 
fit
```

We can see that several factors have a significant association to time to treatment. For example, Factor 1 has a negative coefficient. Samples with low factor values (largely U-CLL) have an increased hazard compared to  with a large factor values (largely M-CLL).

Let's plot the results.
```{r}
s <- summary(fit)
df <- data.frame(factor = factor(rownames(s[["coefficients"]]),
                                 levels = rev(rownames(s[["coefficients"]]))),
...
cut <- survminer::surv_cutpoint(df, variables='Z1')
...
ggLF <- survminer::ggsurvplot(fit, data =df,
                        ...
                        legend = "top",
                        legend.labs = c(paste("low LF 1"), paste("high LF 1")),
