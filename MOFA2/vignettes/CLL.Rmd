...
```{r}
CLL_metadata <- fread("~/Downloads/sample_metadata.txt")
head(CLL_metadata)
```
...
```{r}
# Sanity check
all(sort(CLL_metadata$sample)==sort(unlist(samples_names(MOFAmodel))))

# Add sample metadata to the model
samples_metadata(MOFAmodel) <- CLL_metadata
```
...
```{r}
correlate_factors_with_covariates(MOFAmodel, covariates = c("Gender","died","age"), plot="log_pval")
```
...
By looking at the variance explained plot, we can see that Factor 1 captures variation in all data modalities. Out of all omics, the somatic mutation data is a good place to start, as somatic mutations are very sparse, easy to interpret and any change in the DNA is likely to have downstream consequences to all other molecular layers. Let's plot the weights:
...
Note that some samples are missing the IGHV status because of missing somatic mutation data, but we could predict it! We will play with this later.
```{r}
plot_factor(MOFAmodel, 
  factors = 1, 
  color_by = "IGHV",
...
```{r}
MOFAmodel@samples_metadata$IGHV == pred["IGHV",]
```
...
It seems like Factor 3, 4 and 6 have a strong enrichment for some pathways, but most of the other Factors do not have clear gene set signatures
```{r}
plot_enrichment_heatmap(res)
```
...
```{r}
plot_enrichment(res, factor = 3, max.pathways = 15)
```

It is always advised to visualise which genes are driving the enrichment within each pathways. There are problematic cases where a single gene is driving the enrichment results in very small pathways.
```{r}
plot_enrichment_detailed(MOFAmodel, 
  enrichment.results = res,
  factor = 3, 
  feature.sets = reactomeGS, 
  max.pathways = 3
)
