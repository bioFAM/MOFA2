...
plot_data_overview(sm, show_covariate = T)
```
...
train_opt$seed <- 2021
train_opt$drop_factor_threshold <- 0.02
...
The MOFA model has learnt lengthscales and scales for each factor that tell us how smoothly the factor varies along the covariate: The scale parameter tells us how much of the factors is due to non-structured noise and how much due to structured variation along the covariate. A small lengthscale means that the structured variation has a very rough pattern, a high value tells us that this factor varys very smoothly along the covariate with long-range dependencies along the covariate. In this example, we find two factors that are non-smooth and one smooth factor. Using `plot_factors_vs_cov` we can plot the factors along the covariate to visualize this relationship.
```{r}
get_lengthscales(sm)
get_scales(sm)
