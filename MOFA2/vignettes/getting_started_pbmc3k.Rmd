---
title: "getting_started_pbmc"
description: |
  Tutorial describing how to create and train a MOFA+ model starting from different file types such as AnnData .h5ad file or a Seurat object.
...
We can also define clusters of cells based on their gene expression profiles:

```{r define_cell_clusters}
...
# Set the number of factors to learn to 5 
# for the sake of getting the result faster
factor_model@model_options$num_factors <- 5
# Note that you would typically use more factors in the model

# You can turn the verbose output during training on 
# if you want to monitor the training procedure
...
factor_model@training_options$maxiter <- 100
```
...
The model constructor function `create_mofa()` accepts the data in a multitude of formats including a Seurat object (demonstrated above) and a list of matrices (`matrix`, `dgCMatrix`, or `dgTMatrix`) in R as well as AnnData or loom objects in Python. The information on the latter formats can be found in the respective notebooks, and here's how to use a list of matrices to crate the model:
...
```{r}
plot_weights(factor_model, factors = 1:4)
```

See further vignettes for model investigation and factors interpretation.
