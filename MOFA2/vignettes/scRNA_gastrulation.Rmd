...
This step can take quite some time (~1h without GPU), so we provide a pre-trained model in the next chunk of code.  

If you want to train from scratch, keep in mind that the results will not be 100% reproducible with the pre-trained model.
...
Run t-SNE:
NOTE: Does not work because the model was trained with an older MOFA+ version
```{r}
# set.seed(42)
