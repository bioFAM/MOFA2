...
The data set we use here is a simplified version of the original data set published in (Nature)[https://www.biorxiv.org/content/10.1101/519207v1]. The full data set can be downloaded from [this FTP](ftp://ftp.ebi.ac.uk/pub/databases/scnmt_gastrulation).
...
# load(url("ftp://ftp.ebi.ac.uk/pub/databases/scnmt_gastrulation/mofa2/scnmt_gastrulation/gastrulation_scnmt_mofa.RData"))
```
...
view.colors = view.colors[views(MOFAmodel)]
...
Next step would be to zoom in into those enhancers and characterised them in detail:
...
plot_data_heatmap(MOFAmodel,
  ...
  annotation_col = df,  annotation_colors = list("lineage"=colors),
  cluster_cols = F 
)
```
