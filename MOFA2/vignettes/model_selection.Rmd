...
Details on the various options can be found in the getting_started vignette. Here, we will simply use the default options, changing only the number of factors, and do not make use of the stochastic inference scheme due to the small size of the simulated data set. Default option can be obtained using the functions `get_default_data_options`, `get_default_model_options` and `get_default_training_options`.
...
For details on downstream analyses please have a look at the vignettes `scRNA_gastrulation`, `scMethylation_cortex` and `downstream_analysis`.
