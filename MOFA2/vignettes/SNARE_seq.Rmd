...
```{r, echo=FALSE}
tmp <- metadata %>% copy %>%
  .[,atac_cluster:=NULL] %>% .[,group:="group1"] %>%
  setnames("cell","sample") %>%
  .[sample%in%unlist(samples(mofa))] %>%
  setkey(sample) %>% .[unlist(samples(mofa))]

samples_metadata(mofa) <- tmp
```
...
Plot features with largest (absolute) weight per factor
```{r message=FALSE}
...
```{r}
hist(mofa@data$ATAC[[1]], xlim = c(0,1), breaks=100)
```
...
Let's visualise the actual data (figure below). 
```{r}
plot_data_heatmap(mofa, 
  view = "ATAC", 
  factor = 1, 
...
This looks much better now! a lot of variance is explained by the ATAC topics, even more than for RNA expression. The reason is that the topics provide a compressed representation of the ATAC-seq data based on the Latent Dirichlet Allocation model. 
...
```{r}
cisTopicObject <- getRegionsScores(cisTopicObject, method = "Z-score", scaled = TRUE)
